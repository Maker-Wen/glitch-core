...
static func run(tr) -> void:
	_test_action_preview_does_not_advance_turn(tr)
	_test_action_preview_returns_only_player_action_events(tr)
	_test_action_preview_state_unchanged(tr)
	_test_action_preview_relay_events(tr)

static func _test_action_preview_does_not_advance_turn(tr) -> void:
...
static func _test_action_preview_returns_only_player_action_events(tr) -> void:
	# Set up 4 enemies. Action preview should only contain events related to the
	# attacked unit, NOT each enemy's "next-turn" movement.
	var engine := BattleEngine.new()
	engine.state = BattleState.new()
	engine.state.grid = Grid.new()
	engine.state.phase = BattleState.Phase.PLAYER_ACTION
...
static func _test_action_preview_state_unchanged(tr) -> void:
	var engine := BattleEngine.new()
	engine.state = BattleState.new()
	engine.state.grid = Grid.new()
	engine.state.phase = BattleState.Phase.PLAYER_ACTION
...
static func _test_action_preview_relay_events(tr) -> void:
