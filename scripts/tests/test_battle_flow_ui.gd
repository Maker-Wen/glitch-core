...
static func _test_hud_ability_cooldown_uses_visual_state(tr) -> void:
	...
	tr.assert_eq("ready cooldown skill omits cooldown count badge", ready_slot.get_node_or_null("CooldownCountBadge"), null)
	...
	tr.assert_true("cooling skill shows cooldown count badge", cooling_slot.get_node_or_null("CooldownCountBadge") is Label)
	tr.assert_eq("cooling skill count badge shows remaining rounds", (cooling_slot.get_node("CooldownCountBadge") as Label).text, "1")
	tr.assert_true("cooling skill uses generated muted ring art", cooling_slot.get_node_or_null("CooldownRingMuted") is TextureRect)
	tr.assert_true("cooling skill uses generated progress ring art", cooling_slot.get_node_or_null("CooldownRing") is TextureRect)
	tr.assert_true("cooling skill progress ring uses radial material", (cooling_slot.get_node("CooldownRing") as TextureRect).material is ShaderMaterial)
	var label_texts: Array = []
