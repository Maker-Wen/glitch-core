...
- 音效：`BattleSfxPresenter` 为攻击动作和命中确认播放短促资源化 one-shot，按 `attack_kind` 区分近战、远程推、远程拉和命中质感。音频资源缺失时保持静默，不再使用程序化合成兜底。
