...
动画目标不是“炫”，而是把战斗时序讲清楚。所有动效都应服务三个判断：谁先行动、攻击是否锁定、威胁是否被化解。
...
| 敌方状态预演刷新 | 0.12 秒 | 顺序栈条目从红色平滑变灰，或从红色变为“已移除” | 说明威胁变化 |
...
| 落空攻击 | 0.18 秒 | 灰色攻击线扫到一半断开或偏移消散 | 表达尝试攻击但未命中 |
| 已移除敌人 | 0.12 秒 | 条目显示“已移除”并折叠 | 不浪费攻击动画时间 |
| 条目完成 | 0.1 秒 | 条目右侧出现完成标记，进入下一条 | 表达结算进度 |

落空攻击必须有弱化表现，不能静默跳过。玩家需要看到“这个敌人确实结算了，只是被我化解了”。

### 8.6 状态变化动效

| 状态变化 | 动画表现 |
| --- | --- |
| 命中 → 落空 | 顺序栈条目红色边框退成灰色，攻击线从实线变成灰色虚线 |
| 命中 → 已死亡 | 条目短暂闪红后折叠，棋盘攻击线快速收回 |
| 落空 → 命中 | 条目灰色变红并闪一次，必须明显提示新增风险 |
...
A player hovers an attack target. A translucent amber preview arrow appears. A ghosted enemy final position appears. The enemy action order stack switches into preview mode: one red "will hit" row smoothly turns gray and becomes "neutralized", while another row remains red. The board's corresponding attack line changes from solid red to gray dashed. When the hover is canceled, the preview elements fade out and the UI returns to the original state.
...
9. AI 概念图提示词能够生成含棋盘编号、顺序栈、红色命中、灰色落空四个核心元素的效果图。
